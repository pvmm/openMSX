...
proc read_cdb {fname} {
    ...
    variable a_files
    return [subst [join {
        "[array size c_files] C files added"
        "[array size a_files] assembly files added"
        "[array size c_files_count] C files references added"
        "[array size a_files_count] assembly files references added"
        "$c_count C source lines found"
        "$a_count assembly source lines found"
        "$gf_count global function(s) registered"
        "$sf_count static function(s) registered"
    } "\n" ]]
}
...
proc list_file {file begin {end {}} {focus 0} {showerror 0}} {
    ...
    while {[gets $fh line] >= 0} {
        incr pos
        if {$pos >= $begin} {
            lappend output "[format %-5d $pos][expr {$pos == $focus ? "*" : ":"}]    $line"
        }
        if {$pos >= $end} { break }
    }
    close $fh
...
proc list_address {address {x0 -1} {x1 9} {showerror 0}} {
    ...
    if {$record eq {}} {
        ...
        return "no file for address 0x[h $address]"
    }
