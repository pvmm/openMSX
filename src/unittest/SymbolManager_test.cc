...
{
	CHECK(SymbolManager::checkLabel("foo", 123) == Symbol("foo", 123, -1));
	CHECK(SymbolManager::checkLabel("bar:", 234) == Symbol("bar", 234, -1));
...
{
	CHECK(SymbolManager::checkLabelAndValue("foo", "123") == Symbol("foo", 123, -1));
...
TEST_CASE("SymbolManager: loadLines")
{
	auto dummyParser = [](std::span<std::string_view> tokens) -> std::optional<Symbol> {
		if (tokens.size() == 1) return Symbol{std::string(tokens[0]), 123, -1};
		return {};
	};
