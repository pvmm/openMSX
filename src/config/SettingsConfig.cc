...
void SettingsConfig::loadSetting(const FileContext& context, std::string_view filename)
{
	...
	shortcuts->loadInit();
	for (const auto& shortcut : parser.shortcuts) {
		shortcuts->setShortcut(shortcut.index, shortcut.keychord, shortcut.local, shortcut.repeat);
	}
...
void SettingsConfig::saveSetting(std::string filename)
{
	...
	}
	{
		...
		std::cout << "shortcuts.saveShortcuts(xml);\n";
	}
	xml.end("settings");
}
...
void SettingsParser::attribute(std::string_view name, std::string_view value)
{
	...
	case SHORTCUT:
		if (name == "keychord") {
			if (auto keychord = StringOp::stringTo<ImGuiKeyChord>(value))
...
void SettingsParser::text(std::string_view txt)
{
	...
	case BIND:
		...
		break;
	case SHORTCUT:
		if (auto value = StringOp::stringTo<ShortcutIndex>(txt))
